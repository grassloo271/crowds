...
x1 = jax.random.uniform(key3, crowd_a_size, minval = 0, maxval= 1)
x2 = jax.random.uniform(key4, crowd_a_size, minval = 0, maxval= 1)
xy = jnp.stack([x,y], axis=-1)
xy2 = jnp.stack([x1,x2], axis=-1)
xyz = jnp.stack([xy, xy2])

for ind in range(crowd_a_size):
    crowd_a.append(people(xyz[ind], xy[ind], xy[ind], "tab:blue"))

people_to_npz(crowd_a, "oop")
data_file = "oop.npz"

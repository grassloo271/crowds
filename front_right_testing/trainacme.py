...
def extract_coefficients(model):
    ...
    return {
        "/".join(p.name for p in path): leaf
...
def main(cfg: Config):
    ...
    coeff_arrays = {k: jnp.array([step[k] for step in coefficient_history]) 
                for k in coefficient_history[0]}
    jnp.savez(f"coefficients_{cfg.experiment_name}.npz", **coeff_arrays)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_pedestrians", type=int, default=60)
    parser.add_argument("--eval_fraction", type=float, default=0.02)
    parser.add_argument("--dt", type=float, default=0.1)
    parser.add_argument("--experiment_name", type=str, default="refactored")
    parser.add_argument("--dataset_path", type=str, default="re_cut.npz")
    parser.add_argument("--init_goal_vel_path", type=str, default="goal.npy")
